...
public interface ExamRepository extends JpaRepository<Exam, Long> {
    @Query("SELECT e.id AS id, e.seance AS seance, e.jourNumero AS jourNumero, e.numRooms AS numRooms, e.responsable.id AS responsableId, e.examDate AS examDate FROM Exam e WHERE e.examSession.id = ?1 ")
